...
util.PrecacheSound("ilyusha/jump.mp3")
util.PrecacheSound("ilyusha/panic.mp3")
util.PrecacheSound("ilyusha/spring.mp3")
util.PrecacheSound("ilyusha/stepitup.mp3")
util.PrecacheSound("ilyusha/tooslow.mp3")
util.PrecacheSound("ilyusha/tooeasy.mp3")
util.PrecacheSound("ilyusha/pieceofcake.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_ilyusha_acquire_distance = CreateConVar("npc_ilyusha_acquire_distance", 2500, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which ilyusha will chase a target.")

local npc_ilyusha_spawn_protect = CreateConVar("npc_ilyusha_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, ilyusha will not target players or hide within 200 units of a spawn point.")

local npc_ilyusha_attack_distance = CreateConVar("npc_ilyusha_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of ilyusha's attack.")

local npc_ilyusha_attack_interval = CreateConVar("npc_ilyusha_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between ilyusha's attacks.")

local npc_ilyusha_attack_force = CreateConVar("npc_ilyusha_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of ilyusha's attack. Higher values throw things farther.")

local npc_ilyusha_smash_props = CreateConVar("npc_ilyusha_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, ilyusha will punch through any props placed in their way.")

local npc_ilyusha_hiding_scan_interval = CreateConVar("npc_ilyusha_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "ilyusha will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant ilyushas are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local npc_ilyusha_hiding_repath_interval = CreateConVar("npc_ilyusha_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to ilyusha's hiding spot will be redetermined every X seconds.")

local npc_ilyusha_chase_repath_interval = CreateConVar("npc_ilyusha_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of ilyusha's target will be redetermined every X seconds.")

local npc_ilyusha_expensive_scan_interval = CreateConVar("npc_ilyusha_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local npc_ilyusha_force_download = CreateConVar("npc_ilyusha_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download ilyusha resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear ilyusha!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (npc_ilyusha_force_download:GetBool()) then
	resource.AddWorkshop("174117071")
end

util.AddNetworkString("ilyusha_nag")
util.AddNetworkString("ilyusha_nagresponse")
util.AddNetworkString("ilyusha_navgen")
...
local VECTOR_ilyusha_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_ilyusha_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_ilyusha: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other ilyushas, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC() and
	        ent:Health() > 0 and
	        class ~= "npc_ilyusha" and
	        not class:find("bullseye"))
end

--HACK!!! Because this is an issue a lot of people keep asking me about.
hook.Add("PlayerSpawnedNPC", "ilyushaMissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_ilyusha") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.ilyusha_HasBeenNagged) then return end
	ply.ilyusha_HasBeenNagged = true

	-- Try to explain why ilyusha isn't working.
	net.Start("ilyusha_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("npc_ilyusha: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_ilyusha: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("ilyusha_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.ilyusha_HasBeenNagged = false
		return
	end
	...
	--for _, ilyusha in pairs(ents.FindByClass("npc_ilyusha")) do
	--	ilyusha:Remove()
	--end

	print("npc_ilyusha: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "ilyushaNavGen", navEndGenerate)
	else
		print("npc_ilyusha: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_ilyusha_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `npc_ilyusha_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (npc_ilyusha_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_ilyusha_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. ilyusha means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * npc_ilyusha_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * npc_ilyusha_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not npc_ilyusha_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_ilyusha_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_ilyusha_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "ilyusha/spring.mp3" or "ilyusha/jump.mp3"), 350, 100)
	end
end
...
local tauntSounds = {
	"ilyusha/tooslow.mp3",
	"ilyusha/stepitup.mp3",
	"ilyusha/tooeasy.mp3",
	"ilyusha/pieceofcake.mp3"
}
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > npc_ilyusha_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > npc_ilyusha_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(npc_ilyusha_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > npc_ilyusha_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= npc_ilyusha_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= npc_ilyusha_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= npc_ilyusha_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= npc_ilyusha_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local ilyushasLastFrame = 0
hook.Add("Tick", "ilyushaBenchmark", function()
	local now = SysTime()
	local numilyushas = #ents.FindByClass("npc_ilyusha")

	if (timeAll ~= 0) then

		local ilyushaTime = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    ilyushaTime / ilyushasLastFrame, ilyushaTime, frameTime, (ilyushaTime / frameTime) * 100, ilyushasLastFrame, numThink, (numThink / ilyushasLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	ilyushasLastFrame = numilyushas
end)]]
...
killicon.Add("npc_ilyusha", "ilyusha/npc_ilyusha_killicon", color_white)
language.Add("npc_ilyusha", "Supah ilyusha")
...
local function DevPrint(devLevel, msg)
	if (developer:GetInt() >= devLevel) then
		print("npc_ilyusha: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a ilyusha.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_ilyusha_music_volume = CreateConVar("npc_ilyusha_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by ilyusha. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another ilyusha comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, ilyushas do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_ilyusha_PANIC_COUNT ilyushas are this close, or an equivalent score.
local MUSIC_ilyusha_PANIC_COUNT = 8 -- That's a lot of ilyusha.

local MUSIC_ilyusha_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_ilyusha_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("npc_ilyusha") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("ilyushaPanicMusicUpdate")
	...
	if (panicMusic == nil) then
		if (IsValid(LocalPlayer())) then
			panicMusic = CreateSound(LocalPlayer(), "ilyusha/panic.mp3")
			panicMusic:Stop()
		else
	...
	if (npc_ilyusha_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		panicMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "npc_ilyusha") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_ilyusha_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear ilyusha taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(npc_ilyusha_music_volume:GetFloat(), 0, 1))
...
local function startTimer()
	if (not timer.Exists("ilyushaPanicMusicUpdate")) then
		timer.Create("ilyushaPanicMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local ilyushaMaterial = Material("ilyusha/npc_ilyusha.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(ilyushaMaterial)
...
hook.Add("OnEntityCreated", "ilyushaInitialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_ilyusha") then return end

	local ilyushaEntTable = scripted_ents.GetStored("npc_ilyusha")

	table.Merge(ent, ilyushaEntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("ilyusha_removed", ilyushaDeregister)
end)

hook.Add("NetworkEntityCreated", "ilyushaNetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_ilyusha") then return end
...
surface.CreateFont("ilyushaHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("ilyushaHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("ilyusha is studying this map.", "ilyushaHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "ilyushaHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "ilyushaHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "ilyushaHUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "ilyushaHUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("ilyusha_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "ilyushaNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. ilyusha doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for ilyusha to figure this map out.
...
-- Lazy
net.Receive("ilyusha_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! ilyusha doesn't know this map.
Would you like ilyusha to learn it?]],
		            "This map is currently not ilyusha-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! ilyusha doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to ilyusha.]],
		            "This map is currently not ilyusha-compatible!",
		            "Ok", nagAgain,
